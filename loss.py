...
def mse_loss1(input, label, reduction='mean', name=None):
    ...
    #(paddle.sum(paddle.square(input)))/paddle.sum(input)
    # input=paddle.to_tensor([(paddle.mean(input)),(paddle.sum(paddle.square(input)))/paddle.sum(input)])
    # label=paddle.to_tensor([(paddle.mean(label)),(paddle.sum(paddle.square(label)))/paddle.sum(label)])
    
    Mn_input=paddle.mean(input, axis=2, keepdim=True)
    Mw_input=(paddle.sum(paddle.square(input), axis=2, keepdim=True))/paddle.sum(input, axis=2, keepdim=True)
    input = paddle.concat([Mn_input, Mw_input], axis=2)
    Mn_label=paddle.mean(label, axis=2, keepdim=True)
    Mw_label=(paddle.sum(paddle.square(label), axis=2, keepdim=True))/paddle.sum(label, axis=2, keepdim=True)
    label=paddle.concat([Mn_label, Mw_label], axis=2)
